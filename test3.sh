...
# --- 1. Configuration (Required: REPLACE THESE PLACEHOLDERS) ---
...
# The location ID for the store you want to use (e.g., '01400494' for a test store)
...
get_bearer_token() {
    ...
    if [ -z "${access_token}" ]; then
        ...
        echo "API Response (Error details):" >&2
        # Use jq to pretty-print the error response
        echo "${response}" | jq >&2
        exit 1
    fi
...
search_product() {
    ...
    local encoded_query
    # Note: Bash usually handles URL encoding automatically for simple strings, 
    # but we will manually replace spaces to be safe.
